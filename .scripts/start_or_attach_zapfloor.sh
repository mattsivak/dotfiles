...
if tmux has-session -t zapfloor 2>/dev/null; then
  ...
  alacritty -e tmux new-session -d -s zapfloor \; \
    ...
    split-window -v \; \
    send-keys 'cd ~/code/work/hq-api; bundle exec rake jobs:work' C-m \; \
    split-window -h \; \
    send-keys 'cd ~/code/work/operator-client; nvm exec 18 npm run dev' C-m \; \
    split-window -v \; \
    send-keys 'cd ~/code/work/oc-2/; npm run dev' C-m \; \
    split-window -v \; \
    send-keys 'cd ~/code/work/hq-api; nvim' C-m \; \
    split-window -h \; \
    send-keys 'cd ~/code/work/hq-api; claude' C-m \; \
    split-window -v \; \
    send-keys 'cd ~/code/work/hq-api; lazygit' C-m \; \
    select-layout tiled
