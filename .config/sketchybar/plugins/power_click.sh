...
# NAME is set by sketchybar to the clicked item
PARENT="$NAME"

# Close all other power popups first
for item in power_system power_adapter power_charging power_cpu power_gpu power_ane power_display; do
  if [ "$item" != "$PARENT" ]; then
    $SKETCHYBAR --set "$item" popup.drawing=off
  fi
done

# Toggle popup visibility for the clicked item
...
# Extract adapter values
DELIVERY_WATTS=$(echo "$DATA" | $JQ -r '.charger.delivery_watts')

# Calculate percentages of system
...
# Determine flow icon based on direction
...
# Update power section (using PARENT prefix for item names)
...
$SKETCHYBAR --set ${PARENT}_pp_charge label="Charge: $(printf '%.0f' "$CHARGE_PCT")%" \
...
# Show adapter section if connected
